...
export async function findInstrumentedCommit() {
...
export async function hoistInstrumentedCommit(commitHash) {
    // Check for merge commits between HEAD and the instrumented commit
    const mergeCheck = (await $ `git log --merges ${commitHash}..HEAD --oneline`.text()).trim();
    if (mergeCheck) {
...
export async function getInstrumentedJobs(commitHash) {
...
export async function hasNonInstrumentedChanges(parentBranch, instrumentedCommit) {
    const mergeBase = (await $ `git merge-base ${parentBranch} HEAD`.text()).trim();
    // Count commits between merge-base and the commit before the instrumented one
    const count = (await $ `git rev-list --count ${mergeBase}..${instrumentedCommit}^`.text()).trim();
