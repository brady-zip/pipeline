    ...
    for (const job of jobs.values()) {
        job.needs = job.needs.flatMap((need) => {
            if (need.endsWith(":*")) {
                ...
                return Array.from(wf.jobs.keys()).map((jid) => makeJobKey(workflowName, jid));
            }
            return [need];
        });
    }
    function getDependencies(jobKey) {
        const job = jobs.get(jobKey);
        if (!job)
            return new Set();
        return new Set(job.needs);
    }
    function getRequiredJobs(targets) {
        const required = new Set();
        const visited = new Set();
        const path = [];
        function visit(jobKey) {
            ...
            for (const dep of deps) {
                if (!jobs.has(dep)) {
                    throw new Error(`Missing dependency: ${jobKey} requires ${dep} which does not exist`);
                }
                visit(dep);
            }
            path.pop();
