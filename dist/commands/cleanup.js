...
export const cleanupCommand = new Command("cleanup")
    ...
    const diff = (await $ `git diff ${branchState.parentBranch}...${branchState.testBranch} -- . ':!.github/'`.text()).trim();
    if (!diff) {
        ...
        const result = await $ `git push origin --delete ${branchState.testBranch}`.nothrow();
...
Squashed from test branch (jobs: ${jobList})`;
    ...
    await $ `git branch -D ${branchState.testBranch}`;
    // Try to delete remote branch
    const deleteResult = await $ `git push origin --delete ${branchState.testBranch}`.nothrow();
